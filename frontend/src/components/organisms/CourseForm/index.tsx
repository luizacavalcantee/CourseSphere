...
export const CourseForm = ({ course }: CourseFormProps) => {
  ...
  const onSubmit = async (data: CourseFormData) => {
    ...
    const formattedStartDate = data.start_date.toISOString().split('T')[0];
    const formattedEndDate = data.end_date.toISOString().split('T')[0];
    ...
    const url = isEditMode ? `http://localhost:3001/courses/${course.id}` : 'http://localhost:3001/courses';
    ...
    <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <Input id="start_date" type="date" max="9999-12-31" {...register('start_date')} />
          {errors.start_date && <p className="mt-1 text-sm text-red-500">{errors.start_date.message}</p>}
        </div>
        <div>
          <Label htmlFor="end_date">Data de Término</Label>
          <Input id="end_date" type="date" max="9999-12-31" {...register('end_date')} />
      ...
      <div className="flex gap-4 mt-4">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? 'Salvando...' : (isEditMode ? 'Salvar Alterações' : 'Criar Curso')}
        </Button>
      </div>
    </form>
  );
