...
export const CourseDetails = ({ courseId }: { courseId: string }) => {
    ...
    const { data: course, error: courseError, isLoading: courseLoading } = 
    ...
    if (courseLoading || usersLoading) return <p>Carregando detalhes do curso...</p>;
    if (courseError || usersError) return <p>Falha ao carregar os dados.</p>;
    if (!course) return <p>Curso não encontrado.</p>;

    const instructorNames = users?.filter(u => course.instructors.includes(u.id))
                                                            .map(u => u.name)
                                                            .join(', ');
    ...
    return (
        <div className="bg-white p-8 rounded-lg shadow-md">
            <div className="flex justify-between items-start">
                ...
                {isCreator && (
                    <Link href={`/courses/${course.id}/edit`} className="px-4 py-2 font-semibold text-white bg-indigo-600 rounded-md hover:bg-indigo-700 transition-colors">
                        Editar Curso
                    </Link>
                )}
            </div>
            ...
            <div className="flex items-center gap-6 text-gray-600 mb-6 border-b pb-4">
                <div className="flex items-center gap-2"><Calendar size={16} /> De {formatDate(course.start_date)} até {formatDate(course.end_date)}</div>
                <div className="flex items-center gap-2"><UserIcon size={16} /> Instrutores: {instructorNames || 'Nenhum'}</div>
            </div>
