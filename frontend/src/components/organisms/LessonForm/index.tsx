...
export const LessonForm = ({ courseId, lesson }: LessonFormProps) => {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<LessonFormData>({
    ...
    defaultValues: {
      ...
      publish_date: lesson?.publish_date ? lesson.publish_date.split('T')[0] : '',
    },
  });
  ...
  const onSubmit = async (data: LessonFormData) => {
    ...
    const url = isEditMode ? `http://localhost:3001/lessons/${lesson.id}` : 'http://localhost:3001/lessons';
    ...
    try {
      ...
      toast.success(isEditMode ? 'Aula atualizada!' : 'Aula criada com sucesso!');
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {errors.publish_date && <p className="mt-1 text-sm text-red-500">{errors.publish_date.message}</p>}
        </div>
      </div>
